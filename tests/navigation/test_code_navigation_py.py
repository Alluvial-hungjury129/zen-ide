...
class TestParsePythonImports:
    def test_import_module(self):
        result = _stub._parse_python_imports("import os")
        assert result == {"os": "os"}
    ...
    def test_from_import(self):
        result = _stub._parse_python_imports("from os.path import join")
    ...
    def test_from_import_multiple(self):
        result = _stub._parse_python_imports("from os.path import join, exists")
        assert result == {"join": "os.path.join", "exists": "os.path.exists"}

    def test_relative_import(self):
        result = _stub._parse_python_imports("from .utils import helper")
...
class TestFindPythonSymbolInContent:
    def test_find_class(self):
        content = "import os\n\nclass MyClass:\n    pass"
        assert _stub._find_python_symbol_in_content(content, "MyClass") == 3

    def test_find_function(self):
        content = "def foo():\n    pass"
        assert _stub._find_python_symbol_in_content(content, "foo") == 1
    ...
    def test_not_found_returns_none(self):
        content = "x = 1\ny = 2"
        assert _stub._find_python_symbol_in_content(content, "z") is None
...
class TestFindVariableClass:
    def test_simple_assignment(self):
        ...
        assert _stub._find_variable_class(content, "obj") == "MyClass"

    def test_no_match(self):
        content = "x = 42"
        assert _stub._find_variable_class(content, "x") is None

    def test_lowercase_not_matched(self):
        content = "obj = some_func()"
        assert _stub._find_variable_class(content, "obj") is None

    def test_not_found(self):
        assert _stub._find_variable_class("a = B()", "z") is None

    def test_qualified_class(self):
        content = "client = vault.Client(url)"
        assert _stub._find_variable_class(content, "client") == "vault.Client"
...
class TestFindSelfAttrClass:
    def test_simple_self_attr(self):
        ...
        assert _stub._find_self_attr_class(content, "utils") == "MyUtils"

    def test_self_attr_qualified(self):
        content = "        self.client = vault.Client(url)"
        assert _stub._find_self_attr_class(content, "client") == "vault.Client"

    def test_self_attr_not_found(self):
        assert _stub._find_self_attr_class("self.x = 42", "x") is None

    def test_self_attr_lowercase_rhs_not_matched(self):
        content = "        self.handler = some_factory()"
        assert _stub._find_self_attr_class(content, "handler") is None
    ...
    def test_self_attr_in_init(self):
        ...
        assert _stub._find_self_attr_class(content, "revolve_utils") == "RevolveUtils"
        assert _stub._find_self_attr_class(content, "db") == "DBClient"

    def test_self_attr_no_args(self):
        content = "        self.processor = Processor()"
        assert _stub._find_self_attr_class(content, "processor") == "Processor"
    ...
    def test_self_attr_multiline_content(self):
        ...
        assert _stub._find_self_attr_class(content, "bar") == "SomeClass"

    def test_wrong_attr_not_matched(self):
        content = "        self.foo = MyClass()"
        assert _stub._find_self_attr_class(content, "bar") is None
